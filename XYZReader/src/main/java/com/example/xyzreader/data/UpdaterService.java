...
public class UpdaterService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            ...
            for (int i = 0; i < array.length(); i++) {
                ...
                values.put(ItemsContract.Items.BODY, bodyTextFormatter(object.getString("body")));
    ...
    private String bodyTextFormatter(String rawBody) {
        if (rawBody == null || rawBody.length() == 0) return "";
        rawBody = rawBody.replaceAll("([^\\r\\n])\\R([^\\r\\n])", "$1$2");
        rawBody = rawBody.replaceAll("^ +| +$|( )+", " ").trim();
        rawBody = rawBody.replaceAll("(\\r\\n)", para_separator);
        rawBody = rawBody.replaceAll("(" + para_separator + "){2,}", para_separator);
        return rawBody.replaceAll(para_separator, "\r\n\r\n");
    }
}